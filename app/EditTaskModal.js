...
const EditTaskModal = ({ visible, onClose, task, fetchTasks }) => {
  ...
  const saveChanges = async () => {
    try {
      ...
      const response = await axios.put(`http://192.168.215.172:5000/tasks/${task._id}`, taskData);
  ...
  const deleteTask = async () => {
    try {
      const response = await axios.delete(`http://192.168.215.172:5000/tasks/${task._id}`);
